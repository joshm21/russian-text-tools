...
josh@penguin:~/russian-word-forms-practice/tmp$ rm output.txt
josh@penguin:~/russian-word-forms-practice/tmp$ ls
input.txt  tools.py
josh@penguin:~/russian-word-forms-practice/tmp$ vi tools.py
josh@penguin:~/russian-word-forms-practice/tmp$ python3 tools.py to_accent input.txt > output.txt
josh@penguin:~/russian-word-forms-practice/tmp$ cat tools.py
import csv
import logging
import argparse
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
            ...
            python3 tools.py to_accent input.txt > output.txt""")
    parser.add_argument("tool", help="the tool to run; one of " + ", ".join(tools), choices = tools, metavar="tool")
    parser.add_argument("input", type=argparse.FileType('r'), help="the text to run the tool on")
    parser.add_argument("-l", "--log", help="debug level; defaults to warning; one of " + ", ".join(log_levels), choices= log_levels, metavar="")
    args = parser.parse_args()
    if args.log:
        numeric_level = getattr(logging, args.log.upper(), None)
